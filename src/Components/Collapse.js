            ...
            {openCollapse ? (
                <>
                    {collapseData.map((item) => (
                        <div key={item.id}>
                            <button onClick={() => setOpenCollapse(false)}>
                                <img src={Arrow} alt="Toggle" />
                            </button>
                            <p className='kasa-quality'>{item.title}</p>
                            <p className='kasa-description'>{item.text}</p>
                        </div>
                    ))}
                </>
            ) : (
                <button onClick={() => setOpenCollapse(true)}>{Arrow}</button>
            )}
        </div>
    )
}
